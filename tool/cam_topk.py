...
def cam(path, net, targets, type):
    ...
    cv2.imwrite(join('/home/2021/xiaohui/Storage/Project_code/cam_topk', type, img_path.split('/')[-1].split('.')[0] + '.jpg'), cv2.cvtColor(visualization, cv2.COLOR_RGB2BGR))
    # print(save_path)
...
if __name__ == '__main__':
    ...
    for pre in ['TP', 'TN', 'FP', 'FN']:
        label_csv = pd.read_csv(join('/home/2021/xiaohui/Storage/Project_code', '{}_list.csv'.format(pre)))
        label_csv = label_csv.values.tolist()
        if not os.path.exists('/home/2021/xiaohui/Storage/Project_code/cam_topk/{}'.format(pre)):
            os.makedirs('/home/2021/xiaohui/Storage/Project_code/cam_topk/{}'.format(pre))
        for id in tqdm(range(10)):
            fold = int(label_csv[id][2])
            model = BiCNN()
            state_dict = torch.load('/home/2021/xiaohui/Storage/Project_code/checkpoint/bilinear/2/pre_{}fold.pth.tar'.format(fold))
